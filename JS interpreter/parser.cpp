...
std::vector<Node*>& Parser::ParseToAst() {
	...
	for (auto &token : tokens) {
		...
		if (token.type == TokenType::Number || token.type == TokenType::True || token.type == TokenType::False || token.type == TokenType::String) {
			...
			while (ops.size() > 0 && IsOperator(ops.back()) && operators.at(ops.back()) >= operators.at(token.text)) {
				AddNode(ops.back(), astOutput);
				ops.pop_back();
			}
			ops.push_back(token.text);
...
void Parser::AddNode(std::string op, std::vector<Node*> &astOutput) {
	if (astOutput.size() < 2) {
	...
	astOutput.pop_back();
	astOutput.push_back(new Node(op, left, right));
}
